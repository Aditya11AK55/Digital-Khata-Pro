...
import { getFirestore, doc, setDoc, getDocs, collection, addDoc, updateDoc, query, where, onSnapshot, orderBy } from "https://www.gstatic.com/firebasejs/10.8.0/firebase-firestore.js";
...
let currentShopData = null; // Shop/Dukandar ka data store karne ke liye
...
// --- Auth Logic (Signup with Limit & Plain Password Save) ---
document.getElementById('signup-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // MVP: Save password in Firestore to show during recovery
        await setDoc(doc(db, "shops", userCred.user.uid), { 
            ...
            status: 'Active', // Status for Block/Unblock
            joinDate: new Date()
        });
...
document.getElementById('forgot-password-form').addEventListener('submit', async (e) => {
    ...
    try {
        const q = query(collection(db, "shops"), where("phone", "==", phone), where("pin", "==", pin));
        const querySnapshot = await getDocs(q);

        if (!querySnapshot.empty) {
            const userData = querySnapshot.docs[0].data();
            document.getElementById('shown-password').innerText = `Your Password: ${userData.password}`;
            document.getElementById('recovered-password-display').classList.remove('hidden');
        } else {
            alert("Invalid Mobile Number or Recovery PIN!");
        }
    } catch(err) {
...
document.getElementById('save-new-password-btn').addEventListener('click', async () => {
    ...
    try {
        await updatePassword(auth.currentUser, newPass); // Update Auth System
        await updateDoc(doc(db, "shops", auth.currentUser.uid), { 
            password: newPass // Update database saved password
        });
...
document.getElementById('add-cust-btn').addEventListener('click', () => {
    if(currentShopData) {
        if(allCustomers.length >= currentShopData.limit) {
            // Agar limit poori ho gayi, toh Paywall dikhao
            document.getElementById('premium-modal').classList.remove('hidden');
        } else {
            // Limit bachi hai, normal modal kholo
            openModal();
        }
    }
});
...
// --- Premium Request Handling (Sends request to Super Admin) ---
const requestPremiumPlan = async (planName, price) => {
    try {
        await addDoc(collection(db, "premium_requests"), {
            ...
            status: "Pending" // Super admin ise dekh kar contact karega
        });
...
document.getElementById('save-trx-btn').addEventListener('click', async () => {
    try {
        ...
        await addDoc(trxCollection, { amount: amount, type: currentTrxType, note: note, date: today });
...
// --- Initialization Logic (With Block & Plan Check) ---
onAuthStateChanged(auth, (user) => {
    if (user) {
        // Realtime listener to Shop Document to check Plan, Limit & Block Status
        onSnapshot(doc(db, "shops", user.uid), (shopDoc) => {
            if (shopDoc.exists()) {
                ...
                if (currentShopData.status === 'Blocked') {
                    ...
                    // Display Current Plan Info (Free, Pro, Max)
                    const planInfo = document.getElementById('plan-info');
                    planInfo.innerText = `${currentShopData.plan} Plan`;
                    
                    // Change color based on plan
                    if(currentShopData.plan === 'Free') {
                        planInfo.style.color = "#10b981";
                        planInfo.style.background = "rgba(16, 185, 129, 0.1)";
                    } else {
                        planInfo.style.color = "#4f46e5";
                        planInfo.style.background = "rgba(79, 70, 229, 0.1)";
                    }
